...
public:
    ...
    struct Font
    {
         Font(const wchar_t* fileName, double height,
            bool bold = false,
            bool italic = false,
        ...
        bool   charRect(const wchar_t ch, Rect*rc);
        ...
        HDC                             m_fontDC;
#endif
        FontEngine                      m_fontEngine;       //字体引擎
        FontCacheManager                m_fontCacheManager; //字体缓存
    };
    ...
    void rectangle(double x1, double y1, double x2, double y2, DrawPathFlag flag);
    //圆角矩形
    ...
    // Image Blending (no transformations available)    图像混合（不使用变换）
    ...
    // Copy image directly, together with alpha-channel        直接复制图像，连同alpha通道
