...
namespace agg
{
    ...
    font_engine_win32_tt_base::font_engine_win32_tt_base(bool flag32, 
        ...
        m_font_names(new wchar_t* [max_fonts]),
        m_cur_font(0),

        m_change_stamp(0),
        m_typeface(new wchar_t [256-16]),
        m_typeface_len(256-16-1),
        m_signature(new wchar_t [256+256-16]),
        m_height(0),
        m_width(0),
        m_weight(FW_REGULAR),
        m_italic(false),
    ...
    //------------------------------------------------------------------------
    int font_engine_win32_tt_base::find_font(const wchar_t* name) const
    {
        unsigned i;
        for(i = 0; i < m_num_fonts; ++i)
        {
            if(wcscmp(name, m_font_names[i]) == 0) return i;
        }
        return -1;
    }
                                                ...
                                                glyph_rendering ren_type)
    {
        if(m_dc)
        {
            ...
            if(len > m_typeface_len)
            {
                ...
                m_signature = new wchar_t [len + 32 + 256];
            ...
            else
            {
                ...
                if(m_cur_font)
                {
                    ...
                    update_signature();
                    m_font_names[m_num_fonts] = new wchar_t[wcslen(m_signature) + 1];
                    wcscpy(m_font_names[m_num_fonts], m_signature);
    ...
    void font_engine_win32_tt_base::update_signature()
    {
        ...
        if(m_dc && m_cur_font)
        {
            ...
            swprintf(m_signature, 
                    L"%ls,%u,%d,%d:%dx%d,%d,%d,%d,%d,%d,%08X", 
                    m_typeface,
                    m_char_set,
               ...
               m_glyph_rendering == glyph_ren_agg_gray8)
            {
                ...
                swprintf(buf, L",%08X%08X%08X%08X%08X%08X", 
    ...
    //------------------------------------------------------------------------
    bool font_engine_win32_tt_base::prepare_glyph(wchar_t glyph_code)
    {
        if(m_dc && m_cur_font)
        {
