...
ZAPI(ZuiVoid) ZuiPaintManagerInit(ZuiPaintManager p, HWND hWnd);// 绘图管理器的初始化(m_hWndPaint,m_hDcPaint赋值，在预处理消息中加入管理器)
ZAPI(ZuiVoid) ZuiPaintManagerInvalidate(ZuiPaintManager p);//指定区域失效
ZAPI(ZuiVoid) ZuiPaintManagerInvalidateRect(ZuiPaintManager p, RECT *rcItem);//指定区域失效


ZAPI(ZuiVoid) ZuiPaintManagerSetOpacity(ZuiPaintManager p, BYTE nOpacity);//设置窗口透明度 会开启分层窗口

ZAPI(ZuiVoid) ZuiPaintManagerSetLayered(ZuiPaintManager p, BOOL bLayered);//设置分层窗口

ZAPI(ZuiVoid) ZuiPaintManagerSetLayeredInset(ZuiPaintManager p, RECT *rcLayeredInset);

ZAPI(ZuiVoid) ZuiPaintManagerSetLayeredOpacity(ZuiPaintManager p, BYTE nOpacity);//设置分层窗口透明度

ZAPI(ZuiBool) ZuiPaintManagerAttachDialog(ZuiPaintManager p, ZuiControl pControl);//附加控件树
ZAPI(ZuiBool) ZuiPaintManagerInitControls(ZuiPaintManager p, ZuiControl pControl, ZuiControl pParent);//初始化控件
...
ZAPI(ZuiVoid) ZuiPaintManagerReleaseCapture(ZuiPaintManager p);//释放俘获


ZAPI(ZuiBool) ZuiPaintManagerSetNextTabControl(ZuiPaintManager p, BOOL bForward /*默认TRUE*/);
