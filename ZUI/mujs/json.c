...
static void jsonvalue(js_State *J)
{
    ...
    default:
        js_syntaxerror(J, L"JSON: unexpected token: %ls", jsY_tokenstring(J->lookahead));
    }
}

static void JSON_parse(js_State *J)
{
    const wchar_t *source = js_tostring(J, 1);
    jsY_initlex(J, L"JSON", source);
    jsonnext(J);
    jsonvalue(J);
    // TODO: reviver Walk()
}
...
{
    if (isnan(n)) js_puts(J, sb, L"null");
    else if (isinf(n)) js_puts(J, sb, L"null");
    else if (n == 0) js_puts(J, sb, L"0");
    else {
        wchar_t buf[40];
        swprintf(buf, L"%.17g", n);
        js_puts(J, sb, buf);
    }
}
...
static void fmtindent(js_State *J, js_Buffer **sb, const wchar_t *gap, int level)
{
    js_putc(J, sb, L'\n');
    while (level--)
        js_puts(J, sb, gap);
...
static void fmtobject(js_State *J, js_Buffer **sb, js_Object *obj, const wchar_t *gap, int level)
{
    js_Property *ref;
    int save;
    int n = 0;
...
static void fmtarray(js_State *J, js_Buffer **sb, const wchar_t *gap, int level)
{
    int n, k;
    wchar_t buf[32];
...
static int fmtvalue(js_State *J, js_Buffer **sb, const wchar_t *key, const wchar_t *gap, int level)
{
    ...
    if (js_isobject(J, -1)) {
        if (js_hasproperty(J, -1, L"toJSON")) {
...
void jsB_initjson(js_State *J)
{
    ...
    {
        jsB_propf(J, L"JSON.parse", JSON_parse, 2);
        jsB_propf(J, L"JSON.stringify", JSON_stringify, 3);
    }
    js_defglobal(J, L"JSON", JS_DONTENUM);
}
