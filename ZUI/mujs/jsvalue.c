...
const wchar_t *js_itoa(wchar_t *out, int a)
{
    wchar_t buf[32], *s = out;
    int i = 0;
    while (a) {
        buf[i++] = (a % 10) + '0';
        a /= 10;
...
double js_stringtofloat(const wchar_t *s, wchar_t **ep)
{
    wchar_t *end;
    double n;
    const wchar_t *e = s;
...
double jsV_stringtonumber(js_State *J, const wchar_t *s)
{
    wchar_t *e;
    double n;
...
const wchar_t *jsV_numbertostring(js_State *J, wchar_t buf[32], double f)
{
    wchar_t digits[32], *p = buf, *s = digits;
...
/* ToString() on a value */
const wchar_t *jsV_tostring(js_State *J, js_Value *v)
{
    wchar_t buf[32];
    const wchar_t *p;
    switch (v->type) {
    ...
    case JS_TNULL: return L"null";
    case JS_TBOOLEAN: return v->u.boolean ? L"true" : L"false";
...
static js_Object *jsV_newstring(js_State *J, const wchar_t *v)
{
...
void js_newfunction(js_State *J, js_Function *fun, js_Environment *scope)
{
    ...
    js_pushobject(J, obj);
    {
        ...
        js_defproperty(J, -2, L"length", JS_READONLY | JS_DONTENUM | JS_DONTCONF);
        js_newobject(J);
        {
            js_copy(J, -2);
            js_defproperty(J, -2, L"constructor", JS_DONTENUM);
        }
...
void js_newcfunction(js_State *J, js_CFunction cfun, const wchar_t *name, int length)
{
...
void js_newcconstructor(js_State *J, js_CFunction cfun, js_CFunction ccon, const wchar_t *name, int length)
{
    ...
    js_pushobject(J, obj); /* proto obj */
    {
        ...
        js_defproperty(J, -2, L"constructor", JS_DONTENUM);
        js_defproperty(J, -2, L"prototype", JS_READONLY | JS_DONTENUM | JS_DONTCONF);
    }
}
...
int js_instanceof(js_State *J)
{
    ...
    if (!js_iscallable(J, -1))
        js_typeerror(J, L"instanceof: invalid operand");
    ...
    if (!js_isobject(J, -1))
        js_typeerror(J, L"instanceof: 'prototype' property is not an object");
    O = js_toobject(J, -1);
    js_pop(J, 1);
...
int js_compare(js_State *J, int *okay)
{
    ...
    *okay = 1;
    if (js_isstring(J, -2) && js_isstring(J, -1)) {
        return wcscmp(js_tostring(J, -2), js_tostring(J, -1));
    }
    else {
        double x = js_tonumber(J, -2);
        double y = js_tonumber(J, -1);
...
retry:
    if (JSV_ISSTRING(x) && JSV_ISSTRING(y))
        return !wcscmp(JSV_TOSTRING(x), JSV_TOSTRING(y));
    if (x->type == y->type) {
