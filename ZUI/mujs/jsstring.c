...
static void Sp_toString(js_State *J)
{
	...
	if (self->type != JS_CSTRING) js_typeerror(J, L"not a string");
...
static void Sp_indexOf(js_State *J)
{
	...
	Rune rune;
	while (*haystack) {
		if (k >= pos && !wcsncmp(haystack, needle, len)) {
			js_pushnumber(J, k);
			return;
		}
...
{
	const wchar_t *str = checkstring(J, 0);
	const wchar_t *ss, *ee;
	int len = utflen(str);
...
{
	const wchar_t *source, *needle, *s, *r;
	js_Buffer *sb = NULL;
	int n;
...
{
	const wchar_t *str = checkstring(J, 0);
	const wchar_t *sep = js_tostring(J, 1);
...
void jsB_initstring(js_State *J)
{
	...
	{
		jsB_propf(J, L"String.prototype.toString", Sp_toString, 0);
		jsB_propf(J, L"String.prototype.valueOf", Sp_valueOf, 0);
		jsB_propf(J, L"String.prototype.charAt", Sp_charAt, 1);
		jsB_propf(J, L"String.prototype.charCodeAt", Sp_charCodeAt, 1);
		jsB_propf(J, L"String.prototype.concat", Sp_concat, 0); /* 1 */
		jsB_propf(J, L"String.prototype.indexOf", Sp_indexOf, 1);
		jsB_propf(J, L"String.prototype.lastIndexOf", Sp_lastIndexOf, 1);
		jsB_propf(J, L"String.prototype.localeCompare", Sp_localeCompare, 1);
		jsB_propf(J, L"String.prototype.match", Sp_match, 1);
		jsB_propf(J, L"String.prototype.replace", Sp_replace, 2);
		jsB_propf(J, L"String.prototype.search", Sp_search, 1);
		jsB_propf(J, L"String.prototype.slice", Sp_slice, 2);
		jsB_propf(J, L"String.prototype.split", Sp_split, 2);
		jsB_propf(J, L"String.prototype.substring", Sp_substring, 2);
		jsB_propf(J, L"String.prototype.toLowerCase", Sp_toLowerCase, 0);
		jsB_propf(J, L"String.prototype.toLocaleLowerCase", Sp_toLowerCase, 0);
		jsB_propf(J, L"String.prototype.toUpperCase", Sp_toUpperCase, 0);
		jsB_propf(J, L"String.prototype.toLocaleUpperCase", Sp_toUpperCase, 0);

		/* ES5 */
		jsB_propf(J, L"String.prototype.trim", Sp_trim, 0);
	}
	js_newcconstructor(J, jsB_String, jsB_new_String, L"String", 0); /* 1 */
	{
		jsB_propf(J, L"String.fromCharCode", S_fromCharCode, 0); /* 1 */
	}
	js_defglobal(J, L"String", JS_DONTENUM);
}
