...
static void js_stackoverflow(js_State *J)
{
    ...
    STACK[TOP].u.litstr = L"stack overflow";
    ++TOP;
    js_throw(J);
}
...
static const wchar_t *js_typeof(js_State *J, int idx)
{
    js_Value *v = stackidx(J, idx);
    switch (v->type) {
    default:
    case JS_TSHRSTR: return L"string";
    case JS_TUNDEFINED: return L"undefined";
    case JS_TNULL: return L"object";
    case JS_TBOOLEAN: return L"boolean";
    case JS_TNUMBER: return L"number";
    case JS_TLITSTR: return L"string";
    case JS_TMEMSTR: return L"string";
    case JS_TOBJECT:
...
{
    js_error(J, L"not implemented yet");
}
...
static int jsR_hasproperty(js_State *J, js_Object *obj, const wchar_t *name)
{
    js_Property *ref;
    int k;

    if (obj->type == JS_CARRAY) {
        if (!wcscmp(name, L"length")) {
...
static void jsR_getproperty(js_State *J, js_Object *obj, const wchar_t *name)
{
    if (!jsR_hasproperty(J, obj, name))
        js_pushundefined(J);
}

static void jsR_setproperty(js_State *J, js_Object *obj, const wchar_t *name)
{
    ...
    if (obj->type == JS_CARRAY) {
        if (!wcscmp(name, L"length")) {
            ...
            if (newlen != rawlen)
                js_rangeerror(J, L"array length");
        ...
        if (!wcscmp(name, L"source")) goto readonly;
        if (!wcscmp(name, L"global")) goto readonly;
        if (!wcscmp(name, L"ignoreCase")) goto readonly;
        if (!wcscmp(name, L"multiline")) goto readonly;
        if (!wcscmp(name, L"lastIndex")) {
    ...
    int atts, js_Value *value, js_Object *getter, js_Object *setter)
{
    ...
    if (obj->type == JS_CARRAY) {
        ...
        if (!wcscmp(name, L"ignoreCase")) goto readonly;
        if (!wcscmp(name, L"multiline")) goto readonly;
        if (!wcscmp(name, L"lastIndex")) goto readonly;
    }
...
static int jsR_delproperty(js_State *J, js_Object *obj, const wchar_t *name)
{
    ...
    if (obj->type == JS_CARRAY) {
        ...
        if (!wcscmp(name, L"source")) goto dontconf;
        if (!wcscmp(name, L"global")) goto dontconf;
        if (!wcscmp(name, L"ignoreCase")) goto dontconf;
        if (!wcscmp(name, L"multiline")) goto dontconf;
        if (!wcscmp(name, L"lastIndex")) goto dontconf;
    }
...
const wchar_t *js_ref(js_State *J)
{
    ...
    wchar_t buf[32];
    switch (v->type) {
    case JS_TUNDEFINED: s = L"_Undefined"; break;
    case JS_TNULL: s = L"_Null"; break;
    case JS_TBOOLEAN:
        s = v->u.boolean ? L"_True" : L"_False";
        break;
    case JS_TOBJECT:
        swprintf(buf, L"%p", (void*)v->u.object);
        s = js_intern(J, buf);
        break;
    default:
        swprintf(buf, L"%d", J->nextref++);
        s = js_intern(J, buf);
        break;
    }
    js_setregistry(J, s);
...
static void js_initvar(js_State *J, const wchar_t *name, int idx)
{
    jsR_defproperty(J, J->E->variables, name, JS_DONTENUM | JS_DONTCONF, stackidx(J, idx), NULL, NULL);
}

static void js_defvar(js_State *J, const wchar_t *name)
{
    jsR_defproperty(J, J->E->variables, name, JS_DONTENUM | JS_DONTCONF, NULL, NULL, NULL);
}

static int js_hasvar(js_State *J, const wchar_t *name)
{
...
static void js_setvar(js_State *J, const wchar_t *name)
{
    js_Environment *E = J->E;
    do {
        js_Property *ref = jsV_getproperty(J, E->variables, name);
...
static int js_delvar(js_State *J, const wchar_t *name)
{
    js_Environment *E = J->E;
    do {
...
static void jsR_callfunction(js_State *J, int n, js_Function *F, js_Environment *scope)
{
    ...
    if (F->arguments) {
        ...
        if (!J->strict) {
            ...
            js_defproperty(J, -2, L"callee", JS_DONTENUM);
        }
        js_pushnumber(J, n);
        js_defproperty(J, -2, L"length", JS_DONTENUM);
        for (i = 0; i < n; ++i) {
            js_copy(J, i + 1);
            js_setindex(J, -2, i);
        }
        js_initvar(J, L"arguments", -1);
        js_pop(J, 1);
    }
...
static void jsR_pushtrace(js_State *J, const wchar_t *name, const wchar_t *file, int line)
{
    if (J->tracetop + 1 == JS_ENVLIMIT)
        js_error(J, L"call stack overflow");
    ++J->tracetop;
...
void js_call(js_State *J, int n)
{
    ...
    if (!js_iscallable(J, -n - 2))
        js_typeerror(J, L"called object is not a function");
    ...
    if (obj->type == JS_CFUNCTION) {
        ...
        jsR_pushtrace(J, obj->u.c.name, L"native", 0);
...
void js_construct(js_State *J, int n)
{
    ...
    js_getproperty(J, -n - 1, L"prototype");
    if (js_isobject(J, -1))
...
void js_eval(js_State *J)
{
    if (!js_isstring(J, -1))
        return;
    js_loadeval(J, L"(eval)", js_tostring(J, -1));
    js_rot2pop1(J);
...
void *js_savetrypc(js_State *J, js_Instruction *pc)
{
    if (J->trytop == JS_TRYLIMIT)
        js_error(J, L"try: exception stack overflow");
...
{
    if (J->trytop == 0)
        js_error(J, L"endtry: exception stack underflow");
    --J->trytop;
}
...
static void jsR_run(js_State *J, js_Function *F)
{
    ...
    while (1) {
        ...
        case OP_IN:
            str = js_tostring(J, -2);
            if (!js_isobject(J, -1))
                js_typeerror(J, L"operand to 'in' is not an object");
