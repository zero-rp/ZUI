...
JS_NORETURN void jsC_error(js_State *J, js_Ast *node, const wchar_t *fmt, ...) JS_PRINTFLIKE(3, 4);
...
void jsC_error(js_State *J, js_Ast *node, const wchar_t *fmt, ...)
{
    va_list ap;
    wchar_t buf[512];
    wchar_t msgbuf[256];
    ...
    _snwprintf(buf, 256, L"%ls:%d: ", J->filename, node->line);
    wcscat(buf, msgbuf);
...
static js_Function *newfun(js_State *J, js_Ast *name, js_Ast *params, js_Ast *body, int script)
{
    ...
    F->filename = (wchar_t *)js_intern(J, J->filename);
...
static int addstring(JF, const wchar_t *value)
{
    int i;
    for (i = 0; i < F->strlen; ++i)
        if (!wcscmp(F->strtab[i], value))
            return i;
    if (F->strlen >= F->strcap) {
        F->strcap = F->strcap ? F->strcap * 2 : 16;
        F->strtab = ZuiRealloc(F->strtab, F->strcap * sizeof *F->strtab);
    }
    F->strtab[F->strlen] = (wchar_t *)value;
    return F->strlen++;
}

static void addlocal(JF, js_Ast *ident, int reuse)
{
    const wchar_t *name = ident->string;
    if (J->strict) {
        if (!wcscmp(name, L"arguments"))
            jsC_error(J, ident, L"redefining 'arguments' is not allowed in strict mode");
        if (!wcscmp(name, L"eval"))
            jsC_error(J, ident, L"redefining 'eval' is not allowed in strict mode");
    }
    if (reuse || J->strict) {
        int i;
        for (i = 0; i < F->varlen; ++i) {
            if (!wcscmp(F->vartab[i], name)) {
                if (reuse)
                    return;
                if (J->strict)
                    jsC_error(J, ident, L"duplicate formal parameter '%ls'", name);
            }
        }
    }
    ...
    F->vartab[F->varlen++] = (wchar_t *)name;
}

static int findlocal(JF, const wchar_t *name)
{
    int i;
    for (i = F->varlen; i > 0; --i)
        if (!wcscmp(F->vartab[i - 1], name))
            return i;
    return -1;
}
...
static void emitstring(JF, int opcode, const wchar_t *str)
{
    emit(J, F, opcode);
...
static void emitlocal(JF, int oploc, int opvar, js_Ast *ident)
{
    ...
    if (J->strict && oploc == OP_SETLOCAL) {
        if (!wcscmp(ident->string, L"arguments"))
            jsC_error(J, ident, L"'arguments' is read-only in strict mode");
        if (!wcscmp(ident->string, L"eval"))
            jsC_error(J, ident, L"'eval' is read-only in strict mode");
    }
    if (F->lightweight) {
...
static void emitjumpto(JF, int opcode, int dest)
{
    ...
    if (dest != (js_Instruction)dest)
        js_syntaxerror(J, L"jump address integer overflow");
    emitraw(J, F, dest);
}
...
{
    wchar_t nbuf[32], sbuf[32];
    const wchar_t *needle, *straw;
...
static void cobject(JF, js_Ast *list)
{
    ...
    while (list) {
        ...
        if (prop->type == AST_IDENTIFIER || prop->type == EXP_STRING)
            ...
            jsC_error(J, prop, L"invalid property name in object initializer");
...
static void cassign(JF, js_Ast *exp)
{
    ...
    case EXP_MEMBER:
        ...
        break;
    default:
        jsC_error(J, lhs, L"invalid l-value in assignment");
    }
}
...
static void cassignforin(JF, js_Ast *stm)
{
    ...
    if (stm->type == STM_FOR_IN_VAR) {
        if (lhs->b)
            jsC_error(J, lhs->b, L"more than one loop variable in for-in statement");
    ...
    case EXP_MEMBER:
        ...
        break;
    default:
        jsC_error(J, lhs, L"invalid l-value in for-in loop assignment");
    }
}
...
static void cdelete(JF, js_Ast *exp)
{
    ...
    case EXP_IDENTIFIER:
        if (J->strict)
            jsC_error(J, exp, L"delete on an unqualified name is not allowed in strict mode");
    ...
    case EXP_MEMBER:
        ...
        break;
    default:
        jsC_error(J, exp, L"invalid l-value in delete expression");
    }
}
...
static int matchlabel(js_Ast *node, const wchar_t *label)
{
    while (node && node->type == STM_LABEL) {
        if (!wcscmp(node->a->string, label))
            return 1;
        node = node->parent;
    }
    return 0;
...
static js_Ast *breaktarget(JF, js_Ast *node, const wchar_t *label)
{
    while (node) {
        if (isfun(node->type))
            break;
...
static js_Ast *continuetarget(JF, js_Ast *node, const wchar_t *label)
{
    while (node) {
        if (isfun(node->type))
            break;
...
static void ctrycatch(JF, js_Ast *trystm, js_Ast *catchvar, js_Ast *catchstm)
{
    ...
    L1 = emitjump(J, F, OP_TRY);
    {
        /* if we get here, we have caught an exception in the try block */
        if (J->strict) {
            if (!wcscmp(catchvar->string, L"arguments"))
                jsC_error(J, catchvar, L"redefining 'arguments' is not allowed in strict mode");
            if (!wcscmp(catchvar->string, L"eval"))
                jsC_error(J, catchvar, L"redefining 'eval' is not allowed in strict mode");
        }
...
static void cswitch(JF, js_Ast *ref, js_Ast *head)
{
    ...
    for (node = head; node; node = node->b) {
        ...
        if (clause->type == STM_DEFAULT) {
            if (def)
                jsC_error(J, clause, L"more than one default label in switch");
            def = clause;
        }
        else {
            cexp(J, F, clause->a);
...
static void cstm(JF, js_Ast *stm)
{
    ...
    case STM_BREAK:
        if (stm->a) {
            ...
            if (!target)
                jsC_error(J, stm, L"break label '%ls' not found", stm->a->string);
        }
        else {
            target = breaktarget(J, F, stm->parent, NULL);
            if (!target)
                jsC_error(J, stm, L"unlabelled break must be inside loop or switch");
        }
    ...
    case STM_CONTINUE:
        if (stm->a) {
            ...
            if (!target)
                jsC_error(J, stm, L"continue label '%ls' not found", stm->a->string);
        }
        else {
            target = continuetarget(J, F, stm->parent, NULL);
            if (!target)
                jsC_error(J, stm, L"continue must be inside loop");
        }
    ...
    case STM_RETURN:
        ...
        if (!target)
            jsC_error(J, stm, L"return not in function");
...
static void analyze(JF, js_Ast *node)
{
    ...
    if (node->type == EXP_IDENTIFIER) {
        if (!wcscmp(node->string, L"arguments")) {
            F->lightweight = 0;
            F->arguments = 1;
        }
        else if (!wcscmp(node->string, L"eval")) {
            /* eval may only be used as a direct function call */
            if (!node->parent || node->parent->type != EXP_CALL || node->parent->a != node)
                js_evalerror(J, L"%ls:%d: invalid use of 'eval'", J->filename, node->line);
            F->lightweight = 0;
        }
    }
