...
static js_Property *lookup(js_Property *node, const wchar_t *name)
{
    while (node != &sentinel) {
        int c = wcscmp(name, node->name);
        if (c == 0)
            return node;
        else if (c < 0)
            node = node->left;
...
static js_Property *insert(js_State *J, js_Object *obj, js_Property *node, const wchar_t *name, js_Property **result)
{
    if (node != &sentinel) {
        int c = wcscmp(name, node->name);
        if (c < 0)
...
static js_Property *delete(js_State *J, js_Object *obj, js_Property *node, const wchar_t *name)
{
...
js_Property *jsV_getownproperty(js_State *J, js_Object *obj, const wchar_t *name)
{
    return lookup(obj->properties, name);
}

js_Property *jsV_getpropertyx(js_State *J, js_Object *obj, const wchar_t *name, int *own)
{
    *own = 1;
    do {
...
js_Property *jsV_getproperty(js_State *J, js_Object *obj, const wchar_t *name)
{
    do {
        js_Property *ref = lookup(obj->properties, name);
        if (ref)
...
js_Property *jsV_setproperty(js_State *J, js_Object *obj, const wchar_t *name)
{
    js_Property *result;
    ...
    if (!obj->extensible) {
        ...
        if (J->strict && !result)
            js_typeerror(J, L"object is non-extensible");
        return result;
    }
...
static int itshadow(js_State *J, js_Object *top, js_Object *bot, const wchar_t *name)
{
    int k;
    while (top != bot) {
...
const wchar_t *jsV_nextiterator(js_State *J, js_Object *io)
{
    ...
    if (io->type != JS_CITERATOR)
        js_typeerror(J, L"not an iterator");
    while (io->u.iter.head) {
        js_Iterator *next = io->u.iter.head->next;
        const wchar_t *name = io->u.iter.head->name;
    ...
    if (newlen < obj->u.a.length) {
        if (obj->u.a.length > obj->count * 2) {
            ...
            while ((s = jsV_nextiterator(J, it))) {
                ...
                if (k >= newlen && !wcscmp(s, jsV_numbertostring(J, buf, k)))
