...
{
    const wchar_t *pname = wcsrchr(name, '.');
...
{
    const wchar_t *s = js_tostring(J, 1);
...
static void Encode(js_State *J, const wchar_t *str, const wchar_t *unescaped)
{
    js_Buffer *sb = NULL;
...
static void Decode(js_State *J, const wchar_t *str, const wchar_t *reserved)
{
    js_Buffer *sb = NULL;
    int a, b;
    ...
    while (*str) {
        ...
        if (c != '%')
            ...
            if (!jsY_ishex(a) || !jsY_ishex(b))
                js_urierror(J, L"invalid escape sequence");
            c = jsY_tohex(a) << 4 | jsY_tohex(b);
            if (!wcschr(reserved, c))
                js_putc(J, &sb, c);
            else {
                js_putc(J, &sb, '%');
                js_putc(J, &sb, a);
...
#define URIRESERVED L";/?:@&=+$,"
#define URIALPHA L"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
#define URIDIGIT L"0123456789"
#define URIMARK L"-_.!~*`()"
#define URIUNESCAPED URIALPHA URIDIGIT URIMARK

static void jsB_decodeURI(js_State *J)
{
    Decode(J, js_tostring(J, 1), URIRESERVED L"#");
}

static void jsB_decodeURIComponent(js_State *J)
{
    Decode(J, js_tostring(J, 1), L"");
}

static void jsB_encodeURI(js_State *J)
{
    Encode(J, js_tostring(J, 1), URIUNESCAPED URIRESERVED L"#");
}
...
void jsB_init(js_State *J)
{
    ...
    js_pushnumber(J, NAN);
    js_defglobal(J, L"NaN", JS_READONLY | JS_DONTENUM | JS_DONTCONF);

    js_pushnumber(J, INFINITY);
    js_defglobal(J, L"Infinity", JS_READONLY | JS_DONTENUM | JS_DONTCONF);

    js_pushundefined(J);
    js_defglobal(J, L"undefined", JS_READONLY | JS_DONTENUM | JS_DONTCONF);

    jsB_globalf(J, L"parseInt", jsB_parseInt, 1);
    jsB_globalf(J, L"parseFloat", jsB_parseFloat, 1);
    jsB_globalf(J, L"isNaN", jsB_isNaN, 1);
    jsB_globalf(J, L"isFinite", jsB_isFinite, 1);

    jsB_globalf(J, L"decodeURI", jsB_decodeURI, 1);
    jsB_globalf(J, L"decodeURIComponent", jsB_decodeURIComponent, 1);
    jsB_globalf(J, L"encodeURI", jsB_encodeURI, 1);
    jsB_globalf(J, L"encodeURIComponent", jsB_encodeURIComponent, 1);
}
